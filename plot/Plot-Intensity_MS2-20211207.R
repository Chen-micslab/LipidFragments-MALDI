...
mzSpectrumFilenameList <- list(
    c('2021-11-28/25DHB+CholineChloride-MS2/0_O6/1/104.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-100%EtOH-20210908-MS2/0_D7/1/104.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-50%CHCl3-50%MeOH-20210908-MS2/0_F7/1/104.1000.LIFT/Spectrum_processed.rds'),
    c('2021-12-07/25DHB+Phenylalanine-MS2/0_B15/1/166.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-100%EtOH-20210908-MS2/0_D15/1/166.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-50%CHCl3-50%MeOH-20210908-MS2/0_F18/1/166.1000.LIFT/Spectrum_processed.rds'),
    c('2021-11-28/25DHB+Carnitine-MS2/0_B14/1/184.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-100%EtOH-20210908-MS2/0_D19/1/184.1000.LIFT/Spectrum_processed.rds',
      '2021-11-28/25DHB+SerumExtract-50%CHCl3-50%MeOH-20210908-MS2/0_F21/1/184.1000.LIFT/Spectrum_processed.rds')
)
...
mzAnnotationPeakFilenameList <- list(
    c('2021-11-28/25DHB@MeOH-MS2/0_B8/1/104.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-100%EtOH-20210908-MS2/0_C7/1/104.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-50%CHCl3-50%MeOH-20210908-MS2/0_E7/1/104.1000.LIFT/Spectrum_processed-peaks.csv'),
    c('2021-12-07/25DHB@MeOH-MS2/0_B7/1/166.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-100%EtOH-20210908-MS2/0_C15/1/166.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-50%CHCl3-50%MeOH-20210908-MS2/0_E18/1/166.1000.LIFT/Spectrum_processed-peaks.csv'),
    c('2021-11-28/25DHB@MeOH-MS2/0_B10/1/184.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-100%EtOH-20210908-MS2/0_C19/1/184.1000.LIFT/Spectrum_processed-peaks.csv',
      '2021-11-28/25DHB+SPCS-50%CHCl3-50%MeOH-20210908-MS2/0_E20/1/184.1000.LIFT/Spectrum_processed-peaks.csv')
)
mzAnnotationPeakFilenameList <- addParentDirectory(mzAnnotationPeakFilenameList, 
                                                   mzDataDir)

mzPlotFilename <- '25DHB+Standard+SE1+SE2-MS2_104+166+184.png'
...
mzPlotSubDimensionY <- 3
mzPlotSubDirection <- 'v'
...
mzPlotAxisTickX <- NULL  # MS/MS

mzPlotFontSize <- 50
mzPlotFontSizeTitle <- 16
mzPlotFontSizeAxisLabel <- 30
mzPlotFontSizeAnnotation <- 8

mzPlotLineColorList <- c(rep(list(c('#00A0FF', '#7F0000', '#EEB040')), 3))
mzPlotAnnotationColor <- '#000000'

mzPlotAnnotationText <- '○'
mzPlotAnnotationThreshold <- (mzPlotRangeY[2] - mzPlotRangeY[1]) * 0.02
...
mzAnnotationList <- lapply(mzAnnotationPeakFilenameList, 
                           function(Y){ lapply(Y, 
...
for (i in seq(1, length(mzDataList)))
{
    ...
    for (j in seq(1, length(mzDataList[[i]])))
    {
        ...
        if (j <= length(mzPeakList[[i]]) && j <= length(mzAnnotationList[[i]]))
        {
            # Calculate the position of annotation text
            mzIndexes <- mapUniqueMZ(mzAnnotationList[[i]][[j]][,1], 
                                     mzPeakList[[i]][[j]][,1], 
    ...
    p <- p + theme_bw() + 
        theme(text = element_text(size = mzPlotFontSize),
              ...
              plot.margin = margin(t = 0, r = 2, b = 0, l = -3, unit = 'line')
        )
    p <- p + 
